(function(window, document) {
	var Cupload = function(options) {
		...
		this.imageDownload = new Array();
		this.downloadBtn = new Array();
	...
	Cupload.prototype = {
		...
		createImageBox: function(src, name, width, height, state = true) {
			...
			if(!this.localValue.download){
				...
				this.createImageDownload(src)
				for (var m = 0; m <= this.i; m++) {
					this.imageBox[m].index = m
					this.imageBox[m].onmouseover = function(n) {
						return function() {
							_this.showImageDownload(n)
						}
					}(m)

					this.imageBox[m].onmouseout = function(n) {
						return function() {
							_this.hideImageDownload(n)
						}
					}(m)
				}
			}
		...
		createImageDownload: function(src){
			this.imageDownload[this.i] = document.createElement('div')
			this.imageDownload[this.i].className = 'cupload-image-download'
			this.imageDownload[this.i].style.position = 'absolute'
			this.imageDownload[this.i].style.width = '100%'
			this.imageDownload[this.i].style.height = '100%'
			this.imageDownload[this.i].style.left = 0
			this.imageDownload[this.i].style.top = 0
			this.imageDownload[this.i].style.textAlign = 'center'
			this.imageDownload[this.i].style.verticalAlign = 'middle'
			this.imageDownload[this.i].style.cursor = 'pointer'
			this.imageDownload[this.i].style.color = '#fff'
			this.imageDownload[this.i].style.opacity = 0
			this.imageDownload[this.i].style.background = 'rgba(150,150,150,.5)  no-repeat center center'
			this.imageDownload[this.i].style.backgroundImage = 'url(/resources/images/download.png)'
			this.imageDownload[this.i].style.WebkitTransition = '.3s'
			this.imageDownload[this.i].style.transition = '.3s'
			this.imageDownload[this.i].src = src
			this.imageDownload[this.i].title = '点击下载'
			this.imageBox[this.i].appendChild(this.imageDownload[this.i])
			for (var m = 0; m <= this.i; m++) {
				this.imageDownload[m].onclick = function(n) {
					return function() {
						downFile(src)
					}
				}(m)
		...
		createDeleteBtn: function() {
			...
			this.deleteBtn[this.i].style.fontSize = '30px !important'
			this.deleteBtn[this.i].style.width = '40px'
			this.deleteBtn[this.i].style.height = '40px'
			this.deleteBtn[this.i].style.color = 'white'
			this.deleteBtn[this.i].style.cursor = 'pointer'
			this.deleteBtn[this.i].innerHTML = '×'
			this.deleteBtn[this.i].title = '点击移除'
		...
		showImageDownload: function(m) {
			this.imageDownload[m].style.opacity = 1
		},

		hideImageDownload: function(m) {
			this.imageDownload[m].style.opacity = 0
		},
	}
