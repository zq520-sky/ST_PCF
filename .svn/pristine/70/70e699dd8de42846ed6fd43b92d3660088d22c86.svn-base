            ...
            $(form).ajaxSubmit({
                ...
                success : function(responseText, statusText, xhr, $form) {
                    if (responseText.rs > 0) {
                        top.toastr.success("新增客户信息【" + $("#custNameAdd").val() + "】成功！");
	...
	if (Number(id)) {
		$.ajax({
			...
			success : function(data) {
                ...
                $("#editForm #custCodeHid").val(dataRet.custCode);
                $("#editForm #custCodeEdit").text(dataRet.custCode);
                $("#editForm #loginNameEdit").text(dataRet.loginName);
...
function setPage(id, flag, name) {
    ...
    if (Number(id)) {
        ...
        layer.confirm(msg, {
            ...
            $.get(ctx.path + "/manage/customer/setCustomer"+ ctx.bizSuffix + "?custId=" + id + "&isForbid=" + opr + "&custName=" + name, function(result) {
                if (result.rs == 1) {
